"""
Chotot.com Electric Bicycle Scraper
====================================
Scrapes all electric bicycle (xe đạp điện) listings from Chotot Gateway API.
API: https://gateway.chotot.com/v1/public/ad-listing
Filter: cg=2060 (Xe đạp) + bicycletype=4 (Xe đạp điện)
"""
...
OUTPUT_DIR = r"c:\Ki_5\ev_car\data\raw\chotot"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "bicycles.json")
...
def transform_ad(ad):
    ...
    # Extract bicycletype from params array
    bicycletype_value = ""
    for p in ad.get("params", []):
        if p.get("id") == "bicycletype":
            bicycletype_value = p.get("value", "")
            break

    # Extract bicycle origin from params array
    bicycleorigin_value = ""
    for p in ad.get("params", []):
        if p.get("id") == "bicycleorigin":
            bicycleorigin_value = p.get("value", "")
            break
    ...
    return {
        "url": f"https://xe.chotot.com/mua-ban-xe-dap/{ad.get('list_id', '')}.htm",
        ...
        "ad_params": {
            ...
            "bicycletype": {"value": bicycletype_value},
            "bicycleorigin": {"value": bicycleorigin_value},
            "fuel": {"value": "Điện"},
        },
        "vehicle_type": "xe_dap_dien",
    }


def main():
    print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] Starting electric bicycle scraper...")
    print(f"API: {API_URL}")
    print(f"Filters: cg=2060, bicycletype=4")
