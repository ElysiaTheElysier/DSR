...
class ElectricMotorbikesSpider(scrapy.Spider):
    name = "electric_motorbikes"
    allowed_domains = ["xe.chotot.com"]
    start_urls = ["https://xe.chotot.com/mua-ban-xe-may-dien-sdmt4"]

    def parse(self, response):
        motorbikes_href = response.xpath("//ul/div[@role='button']//a/@href").getall()

        if motorbikes_href:
            for href in motorbikes_href:
                yield response.follow(href, callback=self.parse_motorbike_details)
    ...
    def parse_motorbike_details(self, response):
