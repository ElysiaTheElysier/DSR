...
class ElectricBicyclesSpider(scrapy.Spider):
    name = "electric_bicycles"
    allowed_domains = ["xe.chotot.com"]
    start_urls = ["https://xe.chotot.com/mua-ban-xe-dap-dien-sdbt4"]

    def parse(self, response):
        bicycles_href = response.xpath("//ul/div[@role='button']//a/@href").getall()

        if bicycles_href:
            for href in bicycles_href:
                yield response.follow(href, callback=self.parse_bicycle_details)
    ...
    def parse_bicycle_details(self, response):
